...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qs = [x(next_obs, next_action) for x in self.target_critics]
        ...
        # Predict Q-values
        q_values = torch.stack([x(obs, action) for x in self.critics])
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs_in = obs.unsqueeze(0).repeat(self.num_actor_samples, 1, 1)
            # TODO(student): Compute Q-values for the current state-action pair
            q_values = torch.stack([x(obs_in, action) for x in self.critics])
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values = self.critic(obs, action=action)
