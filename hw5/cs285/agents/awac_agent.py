...
class AWACAgent(DQNAgent):
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            probs = next_action_distribution.probs
            # Use the actor to compute a critic backup

            next_qs = (probs * next_qa_values).sum(dim=-1)

            # TODO(student): Compute the TD target
            target_values = rewards + self.discount * (1-dones) * next_qs
    ...
    def update_actor(
        ...
        with torch.no_grad():
            advantage = self.compute_advantage(observations, actions, action_dist) / self.temperature
            advantage = torch.exp(advantage).detach()
